...
from apps.auth.oauth2 import oauth2_schema
...
async def get_all_users(db: Session = Depends(get_db),token: str = Depends(oauth2_schema)):
    return db_queries.get_all_users(db)


@router.get('/{id}', response_model=schemas.UserDetails)
async def get_user(id: int = None, email: str = None, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
...
async def update_user(id: int, request: schemas.UserCreate, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
...
async def delete_user(id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
